...
export default class extends Controller {
  ...
  submitByKeyboard(event) {
    ...
    const plainEnter =
      event.keyCode == 13 && !event.shiftKey && !event.isComposing;

    if (
      !this.#usingTouchDevice &&
      (metaEnter || (plainEnter && !toolbarVisible))
    ) {
      this.submit(event);
    }
  }
  ...
  #updateFileList() {
    ...
    const fileNodes = this.#files.map((file, index) => {
      ...
      node.className =
        "btn btn--plain composer__file txt-normal position-relative unpad flex-column";
